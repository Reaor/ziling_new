...
function testShapeDragPreservesAndOffsetsMask() {
  ...
  const mask = [{ x: 5, y: 5 }, { x: 6, y: 5 }, { x: 7, y: 5 }];
  ...
  assert.deepEqual(motion.shapeMask, [{ x: 7, y: 6 }, { x: 8, y: 6 }, { x: 9, y: 6 }]);
}
...
function testSetShapeMaskAssignsAllShapeParticipants() {
  ...
  const pool = new CharacterPool(4);
  const motion = new MotionEngine(grid, 16, 0);
  const chars = [pool.acquire('一', 1, 1), pool.acquire('二', 1, 2), pool.acquire('三', 1, 3)];
  for (const char of chars) motion.registerCharacter(char);
  motion._wanderTargets.clear();
  motion.setShapeMask([{ x: 5, y: 5 }, { x: 6, y: 5 }, { x: 7, y: 5 }, { x: 8, y: 5 }], chars.map(char => char.id));
...
function testShapeWanderTargetPrefersBroadMovement() {
  ...
  const mask = [];
  for (let y = 8; y < 15; y++) for (let x = 5; x < 19; x++) mask.push({ x, y });
  motion.setShapeMask(mask, [char.id]);
...
function testShapeTargetRefreshesBeforeLocalBounce() {
  const { motion, char } = createMotionWithOneChar(10, 10);
  const mask = [];
  for (let y = 7; y < 17; y++) for (let x = 3; x < 21; x++) mask.push({ x, y });
  motion.setShapeMask(mask, [char.id]);
...
function testPibtCanPushOccupiedNeighborForward() {
  ...
  motion.registerCharacter(a); motion.registerCharacter(b);
  motion.setTarget(a.id, 3, 1); motion.setTarget(b.id, 3, 1);
  motion.update(200);
  assert.deepEqual(new Set([`${a.gridX},${a.gridY}`, `${b.gridX},${b.gridY}`]), new Set(['2,1', '3,1']));
}
...
function testShapeMotionAllowsSafeConvoyThroughVacatedCells() {
  ...
  const b = pool.acquire('乙', 2, 1);
  motion.registerCharacter(a); motion.registerCharacter(b);
  motion.setShapeMask([{ x: 1, y: 1 }, { x: 2, y: 1 }, { x: 3, y: 1 }, { x: 4, y: 1 }], [a.id, b.id]);
  motion.setTarget(a.id, 4, 1); motion.setTarget(b.id, 4, 1);
  motion.update(200);
  assert.deepEqual(new Set([`${a.gridX},${a.gridY}`, `${b.gridX},${b.gridY}`]), new Set(['2,1', '3,1']));
}
...
function testDisplayRigidBodySeparatesPerpendicularPassThrough() {
  ...
  const b = pool.acquire('乙', 2, 1);
  motion.registerCharacter(a); motion.registerCharacter(b);
  motion.setShapeMask([{ x: 1, y: 1 }, { x: 2, y: 1 }, { x: 2, y: 2 }], [a.id, b.id]);
  motion.setTarget(a.id, 2, 1); motion.setTarget(b.id, 2, 2);
  motion.update(200); motion.updateDisplayPositions(0.5);
  const ax = a.displayX + 8, ay = a.displayY + 8;
  const bx = b.displayX + 8, by = b.displayY + 8;
...
function testNarrowShapeMotionStaysFluidWithoutConflicts() {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    chars.push(char); motion.registerCharacter(char);
  }
  const mask = [];
  for (let y = 8; y < 15; y++) for (let x = 5; x < 19; x++) mask.push({ x, y });
...
function testStationaryCharactersRenderFromCurrentCell() {
  ...
  motion.registerCharacter(char);
  char.prevGridX = 1; char.prevGridY = 0;
  motion.update(200);
...
function testShapeDragClampsShiftWithoutCollapsingMask() {
  const { motion, char } = createMotionWithOneChar(5, 5);
  const mask = [{ x: 20, y: 5 }, { x: 21, y: 5 }, { x: 22, y: 5 }, { x: 23, y: 5 }];
  motion.setShapeMask(mask, [char.id]);
  motion.beginShapeDrag();
...
function testDragRefreshesSomeShapeTargetsDuringMotion() {
  ...
  const before = new Map(chars.map(char => { const target = motion._wanderTargets.get(char.id); return [char.id, `${target?.tx},${target?.ty}`]; }));
