 ...
 *   DEEPSEEK_API_KEY=你的Key node server.js
 *
 * 职责：
 *   1. 托管 index.html / js / css / assets（开发期 HTTP，避免 file:// 模块加载问题）
 *   2. 代理 DeepSeek，提供 POST /api/chat（密钥仅存在环境变量，不进前端）
 *
 * 生产环境：由宿主 App 后端提供等价 API，不使用本文件。
 * 架构说明见：项目架构与集成指南.md
 */
...
http.createServer((req, res) => {
  ...
  // TODO: POST /api/schedule — 见 项目架构与集成指南.md §6.2
  if (url === '/api/schedule' && req.method === 'POST') {
    readBody(req).then((body) => {
      jsonReply(res, 501, { error: 'Not implemented; use mock in frontend for now' });
    }).catch(() => jsonReply(res, 400, { error: 'Invalid request body' }));
    return;
  }

  // TODO: POST /api/validate — 见 项目架构与集成指南.md §6.4
  if (url === '/api/validate' && req.method === 'POST') {
    readBody(req).then(() => {
      jsonReply(res, 501, { error: 'Not implemented' });
    }).catch(() => jsonReply(res, 400, { error: 'Invalid request body' }));
    return;
  }
...
}).listen(8080, () => console.log('ZiLing dev server: http://localhost:8080'));
