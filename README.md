...
**zicodo** —— 把电子宠物「字灵」（汉字精灵）与日程管理结合成的完整 App：
...
- **开屏动画**（随风格而异）：水墨版是千字文小字漂入聚成「字灵」+ 青瓷小印落印；
  现代版是极简的细线与字标。轻触均可跳过。
- **账户与后端（zicodo）**：PWA（可添加到主屏幕）。未登录=本地模式（数据存本机，全功能可用）；
  在「我的 → 登录/注册」填入自托管的 [zicodo 后端](https://github.com/qiadastrachen-bit/zicodo)地址并登录后：
  任务云同步（`title/dueDate/category/status` 映射，`/check` 打卡）、**真实多人团队**
  （6 位邀请码加入、成员/角色/积分、全服排行，`team_members` 多对多）、字灵 AI 直连
  `/ziling/api/*`（`js/ai/bridge.js` 自动适配 `{code,data}` 包裹并带 JWT）。
- **任务提醒/闹钟**：按条开启（需选时间），到点应用内响铃卡片（完成/+10分/知道了）+
  青瓷三连音 + 震动；授权后后台弹系统通知。纯前端实现（zicodo 暂无提醒字段）。
- **多团队**：本地模式即可多团队（任务归队、按队统计）；登录后切换为服务器真实团队。
